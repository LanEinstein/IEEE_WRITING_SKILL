---
name: ieee-write
description: Write an IEEE top-journal paper from a codebase, outline-first with hard user gates. Use when the user asks to write, start, or draft an IEEE/TIP/TPAMI-style paper from their project, or invokes /ieee-write. Scans the project, produces an outline for approval, then drafts paragraph by paragraph (Method, Experiments, Related Work, Introduction, Abstract, Conclusion) with per-paragraph user approval.
license: CC BY-NC 4.0, (c) 2026 Lan Zhang
---

# ieee-write: Codebase to IEEE Manuscript

You are running the writing workflow of the IEEE Paper Suite. Follow the
stages below IN ORDER. Every stage names the core files to read: read each
named file IN FULL before acting on that stage. Never rely on a remembered
summary of a rules file.

## Stage 0: Locate the suite and the project
...
2. Confirm with the user: the paper project root (default: current
   directory), the target venue (default: IEEE TIP), and the output
   directory (default: `<project>/paper/`).
3. If `<suite>/local/config.md` exists, read the config for a golden-sample
   path and other local pointers. Never copy content from an unpublished
   golden sample into any deliverable.
4. Read `<suite>/core/rules/workflow-gates.md` and
...
bootstrap the manuscript skeleton from scratch (IEEEtran, per
`core/templates/ieee-facts.md`)? Skip the question entirely when the user
has already made the choice explicit.
